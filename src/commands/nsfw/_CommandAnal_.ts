...
export const _CommandAnal_: PCommand = {
    category: "NSFW",
    aliases: [],
    ...
    data: new SlashCommandBuilder()
        .setName("anal")
        .setDescription("Returns a lewd NSFW picture involving anal!"),
    execute: async (client: Client, interaction: CommandInteraction | any) => {
        if(!interaction.channel?.nsfw) return void void interaction.followUp({ content: null, embeds: [errorEmbed(`This channel is not marked as a **\`NSFW\`** channel!`)] });
        got("https://nekobot.xyz/api/image?type=anal").then(response => {
